...
Description: Comprehensive Exploratory Data Analysis script covering structural metadata,
             data quality, univariate statistics, correlation matrices, visual suites,
             index analysis, and domain-specific feature engineering.
"""
...
def load_and_inspect_dataset(file_path: str) -> pd.DataFrame:
    ...
    if not os.path.exists(file_path):
        raise FileNotFoundError(
            f"Dataset not found at path: {file_path}"
        )
...
def analyze_data_quality(df: pd.DataFrame) -> None:
    ...
    # Elevation must physically remain between 0 and 90 degrees; Cn0DbHz typically between 10 and 60 dB-Hz
    invalid_elevation = (
        df[df["SvElevationDegrees"] < 0] if "SvElevationDegrees" in df.columns else None
    )
    suspicious_cn0 = (
        df[(df["Cn0DbHz"] < 10) | (df["Cn0DbHz"] > 60)] # cant be more than 60 because the signal is beneath the noise floor
        # and cant be  less than 10 because is the tracing threshold
        if "Cn0DbHz" in df.columns
        else None
    )

    print(
        f"4.3 Suspicious Elevation Entries (<0 deg): {0 if invalid_elevation is None else len(invalid_elevation):,}"
    )
    print(
        f"4.3 Suspicious C/N0 Signal Entries (<10 or >60 dB-Hz): {0 if suspicious_cn0 is None else len(suspicious_cn0):,}"
    )
...
def univariate_numerical_analysis(df: pd.DataFrame, column: str) -> None:
    ...
    print(
        f"MAD:  {mad_val:.2f}    | IQR:    {iqr_val:.2f}  | Skewness: {skewness:.2f}"
    )
    print(
    ...
    outliers_iqr = series[
        (series < (q1 - 1.5 * iqr_val)) | (series > (q3 + 1.5 * iqr_val))
    ]
    ...
    mod_z_scores = (
        0.6745 * np.abs(series - median_val) / (median_absolute_deviation + 1e-9)
    )
...
# ==============================================================================
# SECTION 6: CORRELATIONS, CRAMÉR'S V & VISUALIZATIONS SUITE
# ==============================================================================
...
def analyze_correlations_and_relationships(df: pd.DataFrame) -> None:
    ...
    contingency = pd.crosstab(
        df_binned["SignalType"], df_binned["Signal_Strength_Bin"]
    )
    v_stat = calculate_cramers_v(contingency)

    print("-" * 70)
    print(
        f"6.2 Cramér's V Correlation (SignalType vs Binned C/N0): {v_stat:.3f}"
    )
    print("=" * 70 + "\n")
...
def plot_comprehensive_visualizations(df: pd.DataFrame) -> None:
    ...
    clean_gps = df[
        (df["ConstellationType"] == 1) & (df["SignalType"] == "GPS_L1_CA")
    ].copy()

    # 1. Scatterplot: Elevation vs C/N0 (Physical LOS Correlation)
    ...
    plt.title(
        "Scatterplot: Signal Strength (C/N0) vs. Elevation Angle (GPS L1)",
        fontsize=14,
    )
    ...
    sns.histplot(
        ...
        kde=True,
        color="teal",
        label="C/N0 Frequency",
    )
    plt.title(
        "Histogram: Distribution of Carrier-to-Noise Density (C/N0)",
        fontsize=14,
    )
    ...
    # 4. barplot: elvaion mean by Constellation Type
    const_std = (
        df.groupby("ConstellationType")["SvElevationDegrees"].mean().reset_index()
    )
    ...
    const_std = (
        df.groupby("ConstellationType")["Cn0DbHz"].mean().reset_index()
    )
    ...
    plt.title(
        "Boxplot: Signal Strength Across Different GNSS Constellations",
        fontsize=14,
    )
    ...
    plt.show()
    # 5. Violinplot: Elevation Angle Distribution across Signals
    plt.figure(figsize=(10, 5))
    sns.violinplot(
        data=df, x="SignalType", y="SvElevationDegrees", palette="mako"
    )
    ...
    plt.tight_layout()
    plt.show()
    # 8. Heatmap: Spearman Correlation Matrix
    ...
    plt.tight_layout()
    plt.show()
    # 9. Scatterplot: time vs C/N0 (show the c/n0 of the most seen svid)
    ...
    plt.figure(figsize=(10, 5))
    sns.scatterplot(
        data=svid_max_time_appers,
        x=(svid_max_time_appers["TimeNanos"] / 1e9)/60,
        y="Cn0DbHz",
        alpha=0.4,
        color="dodgerblue",
        label="Observed Measurements",
    ...
    plt.title(
        "SV C/N0 vs Time (SV with the max time appers",
        fontsize=14,
    )
    ...
    plt.tight_layout()
    plt.show()
# 10. Scatterplot: time vs C/N0 (svid's with the min and max std in the c/n0)
    gps_l1 = df[
        (df["ConstellationType"] == 1) & (df["SignalType"] == "GPS_L1_CA")
        ].copy()
    # delete the satellite that appers lees then 50 time to improve the data
    valid_svids = (
        gps_l1.groupby("Svid")["Cn0DbHz"].count()[lambda x: x >= 50].index
    )
    gps_l1_filtered = gps_l1[gps_l1["Svid"].isin(valid_svids)]

    # calculate std of each satellite
    std_per_svid = gps_l1_filtered.groupby("Svid")["Cn0DbHz"].std()

    # find svid with the max std and min std
    ...
    plt.title(
        "SV C/N0 vs Time (SV with the max std in the c/n0)",
        fontsize=14,
    )
    plt.xlabel("time [min]", fontsize=12)
    plt.ylabel("C/N0 [dB-Hz]", fontsize=12)
    ...
    plt.title(
        "SV C/N0 vs Time (SV with the min std in the c/n0)",
        fontsize=14,
    )
    plt.xlabel("time [min]", fontsize=12)
    plt.ylabel("C/N0 [dB-Hz]", fontsize=12)
    ...
    # 10. Scatterplot: Elevation vs time (svid with the max and min std in the c/n0)
    ...
    plt.title(
        "SVSvElevationDegrees vs Time (SV with the max std in the c/n0)",
        fontsize=14,
    )
    plt.xlabel("time [min]", fontsize=12)
    plt.ylabel("Elevation Angle [Degrees]", fontsize=12)
    ...
    plt.title(
        "SvElevationDegrees vs Time (SV with the min std in the c/n0)",
        fontsize=14,
    )
    plt.xlabel("time [min] ", fontsize=12)
    plt.ylabel("Elevation Angle [Degrees]", fontsize=12)
...
def inspect_index_structure(df: pd.DataFrame) -> None:
    ...
    print(
        f"Is DataFrame Index Monotonically Increasing? -> {df.index.is_monotonic_increasing}"
    )
    ...
    if "ReceivedSvTimeNanosSinceGpsEpoch" in df.columns:
        ...
        print(
            f"Is Dataset Monotonically Sorted by GPS Time? -> {is_time_sorted}"
        )
    print("=" * 70 + "\n")


# ==============================================================================
# SECTION 9: BONUS - DOMAIN FEATURE ENGINEERING (10%+)
# ==============================================================================
...
def engineer_domain_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Logical Engineering Rule: High elevation (>=40 deg) AND Strong carrier-to-noise ratio (>=35 dB-Hz)
    enhanced_df["Is_Reliable_LOS"] = np.where(
        (enhanced_df["SvElevationDegrees"] >= 40)
        & (enhanced_df["Cn0DbHz"] >= 37),
        1,
        0,
    )
    ...
    print(
        f"Reliable Line-of-Sight (LOS) Measurements in Dataset: {los_percentage:.2f}%"
    )
    print("=" * 70 + "\n")
    return enhanced_df
...
def main():
    ...
    # Step 1: Load and Inspect Dataset (Section 3)
    dataset_path = "device_gnss_train_p4xl.csv"
    gnss_df = load_and_inspect_dataset(dataset_path)

    # Step 2: Quality and Integrity Analysis (Section 4)
    analyze_data_quality(gnss_df)

    # Step 3: Univariate Analysis and Outliers Detection (Section 5)
    univariate_numerical_analysis(gnss_df, "Cn0DbHz")

    # Step 4: Inter-Variable Correlations and Relationships (Section 6.1 & 6.2)
    analyze_correlations_and_relationships(gnss_df)

    # Step 5: Comprehensive Visualizations Suite (Section 6.3)
    plot_comprehensive_visualizations(gnss_df)

    # Step 6: Index Analysis (Section 7)
    inspect_index_structure(gnss_df)

    # Step 7: Bonus Feature Engineering (Section 9)
